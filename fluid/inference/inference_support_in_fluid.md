...
## C++ Inference API
- **多线程共享Parameters [:arrow_forward:](https://github.com/PaddlePaddle/Paddle/blob/develop/paddle/fluid/inference/tests/test_multi_thread_helper.h)**<small><small>
  - 主线程
    - :zero: 初始化设备
    - :one: 定义`place`，`executor`，`scope`
  ...
  - 从线程
    ...
    - :four::five: 准备feed数据，定义Tensor来fetch结果
    - :six: 执行`copy_program`
      ```cpp
